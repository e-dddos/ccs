...
void save_obstacles(struct Robot *myrobot)
{
    ...
    for(i = 0; i < 4; i++){
        if(!(myrobot->free_directions[i])){
            myrobot->room[myrobot->x_pos + offset[myrobot->direction + i][0]][myrobot->y_pos + offset[myrobot->direction + i][1]] = 255;
        }
    }
}
